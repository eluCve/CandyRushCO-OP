...
function gameStart(character) {
  ...
  paragraph.innerHTML = "Move players with WASD and Arrow Buttons";
  document.getElementById("buttons").appendChild(paragraph);
  paragraph.id = "info";
  ...
  function updateGameArea() {
    ...
    if (myGameArea.frameNo == 0 || everyinterval(170)) {
      ...
      gap = 200;
      myObstacles.push(
        new object(100, height, x, 0, rotatedcandy, "character")
      );
      myObstacles.push(
        new object(100, y - height - gap, x, height + gap, candy, "character")
      );
    }
    ...
    if (myGamePiece.crashWith(myGamePieceSecond)) {
      var minBounce = 20;
      var maxBounceX =
        (Math.abs(myGamePiece.speedX) + Math.abs(myGamePieceSecond.speedX)) *
        5 +
        minBounce;
      var maxBounceY =
        Math.abs(myGamePiece.speedY) +
        Math.abs(myGamePieceSecond.speedY) * 5 +
        minBounce;
